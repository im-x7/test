    ...
    <t-card class="filter-card" :bordered="false">
      <t-form layout="inline">
        <t-form-item label="分类">
          <t-select v-model="filters.category" placeholder="请选择分类" style="width: 200px" clearable>
            <t-option value="graffiti_tattoo">涂鸦背景纹身</t-option>
            <t-option value="black_tattoo">黑色背景纹身</t-option>
            <t-option value="graffiti_accessory">涂鸦背景饰品</t-option>
            <t-option value="black_accessory">黑色背景饰品</t-option>
          </t-select>
        </t-form-item>
        ...
        <t-form-item style="margin-left: auto">
...
const filteredData = computed(() => {
  return albumData.value.filter(item => {
    const matchText = item.title.includes(filterText.value) || 
      (Array.isArray(item.keywords) && item.keywords.some(kw => kw.includes(filterText.value)))
    return matchText
  })
})
...
const tableData = computed(() => {
  const groups: { [key: string]: { category: string; type: string; tattoo: AlbumImage[]; cover: AlbumImage[]; design: AlbumImage[]; createTime: string } } = {};
  ...
  filteredData.value.forEach(item => {
    ...
    if (!groups[title]) {
      groups[title] = {
        category: title,
        type: item.category === 'tattoo' ? '纹身' : item.category === 'cover' ? '遮盖' : '设计',
        tattoo: [],
        cover: [],
        design: [],
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (actualFile && (actualFile instanceof Blob || actualFile instanceof File)) {
          ...
          resolve({
            status: 'fail', // 使用fail而不是error，符合TDesign的状态要求
...
const handleEditGroup = (row: TableRow) => {
  ...
  currentGroup.value = {
    category: row.type,
    tattooFiles: row.tattoo.map(img => ({
      name: img.title,
      url: img.url,
      status: 'done'
    })),
    coverFiles: row.cover.map(img => ({
      name: img.title,
      url: img.url,
      status: 'done'
    })),
    designFiles: row.design.map(img => ({
      name: img.title,
      url: img.url,
      status: 'done'
    }))
...
const handleUpload = async () => {
  ...
  // 检查是否所有组都选择了分类
  const hasEmptyCategory = uploadGroups.value.some(group => 
    (group.tattooFiles.length > 0 || group.coverFiles.length > 0 || group.designFiles.length > 0) && !group.category
  );
  
  if (hasEmptyCategory) {
    MessagePlugin.warning('请为所有上传的图片选择分类');
    return;
  }
    ...
    const uploadPromises = uploadGroups.value.flatMap(group => {
      ...
      promises.push(...group.tattooFiles.map(async (fileObj) => {
        try {
          ...
          const result = await uploadFile(file, group.category, 'tattoo');
          return result;
        } catch (error) {
      ...
      promises.push(...group.coverFiles.map(async (fileObj) => {
        try {
          ...
          const result = await uploadFile(file, group.category, 'cover');
          return result;
        } catch (error) {
      ...
      promises.push(...group.designFiles.map(async (fileObj) => {
        try {
          ...
          const result = await uploadFile(file, group.category, 'design');
          return result;
        } catch (error) {
      ...
      albumData.value = [
        {
          ...
          keywords: ['龙', '传统'],
          url: 'https://qcloud.dpfile.com/pc/JOnN4hstpi3_PAYQ8iDQaMPdCGvNvIOF8hXFpYtdqjMXyPEkpUqfdIUZwCbbB1Xz.jpg'
        },
        {
          id: 2,
          title: '龙纹身-纹身照片-2',
        ...
        },
        {
          ...
          category: 'design',
          group: '凤凰纹身',
          createTime: '2023-05-16 10:35:00',
          keywords: ['凤凰', '设计'],
          url: 'https://img0.baidu.com/it/u=2028084904,3939052004&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500'
        }
      ]
